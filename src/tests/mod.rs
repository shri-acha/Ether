...
#[test]
fn parser_match_with_literals() {
    let code = r#"
fn test_number(n: int): int {
    match (n) {
        1 => {
            return 100;
        }
...
"#;
    ...
    assert_matches!(program, Ok(_));
    let program = program.unwrap();
    
    // Check that we have a function
    assert_eq!(program.declarations.len(), 1);
    if let Declaration::Function(func) = &program.declarations[0] {
        assert_eq!(func.header.name, Some("test_number".to_string()));
        
        // Check that the function body contains a match statement
        assert_eq!(func.body.statements.len(), 1);
        if let crate::parser::Stmt::Match { scrutinee, arms } = &func.body.statements[0] {
            // Check that we have 3 arms
            assert_eq!(arms.len(), 3);
            
            // Check first arm pattern is literal 1
            if let crate::parser::Pattern::Literal(crate::parser::Literal::Int(val)) = &arms[0].pattern {
                assert_eq!(val, "1");
            } else {
            ...
            if let crate::parser::Pattern::Identifier(name) = &arms[2].pattern {
                assert_eq!(name, "_");
            } else {
...
"#;
    ...
    if let Declaration::Function(func) = &program.declarations[1] {
        if let crate::parser::Stmt::Match { scrutinee, arms } = &func.body.statements[0] {
            ...
            // Check that all arms have enum variant patterns
            for (i, arm) in arms.iter().enumerate() {
                if let crate::parser::Pattern::EnumVariant(enum_name, variant_name) = &arm.pattern {
                    assert_eq!(enum_name, "Status");
                    match i {
                        0 => assert_eq!(variant_name, "Ok"),
                        1 => assert_eq!(variant_name, "Error"),
                        2 => assert_eq!(variant_name, "Pending"),
...
#[test]
fn parser_match_with_mixed_patterns() {
    let code = r#"
fn test_mixed(x: int): bool {
    match (x) {
        0 => {
            return true;
        }
